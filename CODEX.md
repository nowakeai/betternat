...
- Fallback datapath: none. nftables code may remain temporarily as legacy
...
- `internal/datapath/nftables/`: legacy nftables diagnostic wrapper retained
  temporarily; do not add new fallback behavior.
...
- Linux is required for real LoxiLB datapath execution. Any suitable Linux host
  ...
  legacy/diagnostic only while it remains.
