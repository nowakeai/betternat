...
- bootstrap-only changes that preserve topology and ownership use an ASG launch-template rollout,
- runtime, datapath, HA timing, route, subnet, AMI, and EIP-mode changes require explicit replacement,
...
| Bootstrap-only change that preserves topology and ownership | In-place ASG launch-template rollout. |
| Runtime version, AMI, datapath, subnet, route, or EIP-mode change | Replacement. |
...
### Preserve the public identity first

The recommended production model is an independent `aws_eip` resource passed
through `eip_allocation_ids`. The gateway associates that address but never
releases it, so an explicit gateway replacement cannot delete the public
identity.

For an existing provider-managed EIP, first enable retention and apply the
state-only change:

```hcl
retain_managed_eips_on_destroy = true
```

Then run the explicit replacement. Destroy leaves the tagged EIP in place and
the same-name create adopts it. Verify `egress_public_ips` before and after the
replacement. To retire the gateway and its provider-managed EIP permanently,
set retention back to `false`, apply, and then destroy.

Do not use the retention option as the primary ownership model for new
production deployments. An independent `aws_eip` is visible in Terraform even
while the gateway resource does not exist and supports explicit
...
- prepare the route table IDs and fallback targets needed by the
  [Rollback Guide](ROLLBACK_GUIDE.md#emergency-route-restore),
...
Use the [Rollback Guide](ROLLBACK_GUIDE.md#emergency-route-restore) for manual
...
See [Rollback Guide](ROLLBACK_GUIDE.md) for the full destroy and rollback procedure.
