...
NAT-layer monthly estimate only. This table compares the NAT Gateway-specific bill line against the BetterNAT appliance-specific bill line. It does not include standard AWS data transfer:

| Monthly traffic through NAT layer | NAT Gateway-specific cost | BetterNAT-specific cost | NAT-layer savings | Savings % |
| ---: | ---: | ---: | ---: | ---: |
...
The BetterNAT-specific column includes only illustrative appliance instance hours: `2 appliances * $0.05/hour * 730 hours = $73/month`. Your total AWS bill still includes direction-dependent standard data transfer, plus EBS, EIP/public IPv4, DynamoDB, monitoring, and operational cost. Upload-heavy workloads still pay normal AWS internet egress charges in both designs; download-heavy workloads often feel the largest improvement because NAT Gateway was adding a per-GB processing charge to return traffic that may otherwise have little or no AWS data-transfer-in charge. See [Cost Model](docs/user/COST_MODEL.md) for formulas, direction examples, caveats, and CLI usage.
